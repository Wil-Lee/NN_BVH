...
import nss_kd_tree
import nss_common
import nss_callbacks
import numpy as np
import tensorflow as tf
import nss_neural_spatial_split
...
class neural_kdtree :
    ...
    def save_variables(self) :
        ...
        nss_common.clear_make_dir(outDir)
    ...
    def predict(self, point_clouds, useGreedyInference) :
        ...
        for i, pc in enumerate(input_pc) :
            input_pc[i] = nss_common.applyNormalization(pc, nss_common.getAABBox(pc), 1.0)
    ...
    def test_model(self, pDataCSV, pc_dir) :
        valid_ds = nss_data_stream.pointcloud_stream(pc_dir, pDataCSV, self.mBatchSize, False)

        validGen = valid_ds.get_dataset()

        sah_tree = nss_kd_tree.kd_tree(pMaxLevels=self.treeLevels,
            pMaxLeafCapacity=0,
            pStrategy= nss_kd_tree.strategy.SURFACE_HEURISTIC_RECURSIVE)
