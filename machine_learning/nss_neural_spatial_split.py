...
class spatialSplit_Model(keras.Model) :
    ...
    def __make_empty_tree_2wide(self) :
        ...
        root_node = nss_tree_common.neural_spatial_node('root.', 0)
        ...
        for lvl_i in range(1, self.treeLevels) :
            ...
            for node_i in range(2 * len(self.greedy_nodes[self.lvl(lvl_i - 1)])) :
                ...
                node = nss_tree_common.neural_spatial_node(
    ...
    def __make_empty_tree_6wide(self) :
        ...
        root_node = nss_tree_common.neural_spatial_node('root.', 0)
        # self.lvl(0) converts the number argument to a string
        self.neural_nodes[self.lvl(0)] = [root_node,]
        # tf.Variable defines a variable with can be updated during training
        # tf.ones returns a tensor of 1's with dimension: rows=batch_size, columns= 3
        # afterwards the tensor gets multiplied by 0.5 so every entry is 0.5
        ...
        for lvl_i in range(1, self.treeLevels) :
            ...
            for node_i in range(6 * len(self.neural_nodes[self.lvl(lvl_i - 1)])) :
                parent_node = self.neural_nodes[self.lvl(lvl_i - 1)][node_i // 6] # node_i // 6 floor div

                node = nss_tree_common.neural_spatial_node(
                    parent_node.name + self.name_map[node_i % 6], lvl_i)

                # set values of node to random number in [0,1)
    ...
    def train_step(self, epoch, step, input) :
        ...
        # converts tf.Elements back to numpy arrays
    ...
    def greedy_predict_tree(self, point_clouds) :
        ...
        for lvl_i in range(self.max_inter_lvl) :
            ...
            for node_i, node in enumerate(nodes) :
                ...
                node_mask = nss_tree_common.build_mask(point_clouds, node.bounds)
