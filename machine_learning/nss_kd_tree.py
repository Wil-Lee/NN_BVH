...
class treeNode() :
    def __init__(self, name, points, parentSplitDim, level, primitive_tp, aabb = None, parent_node=None, indices=None) :
        self.name = name
        self.aabb = nss_common.getAABBox(points) if aabb is None else aabb
...
class kd_tree() :
    def __init__(self,
        ...
        self.max_splits, self.max_nodes = nss_common.num_splits_nodes(self.maxlevels - 1)
        self.nodes_map = dict()
        self.nodes_arr = list()
    ...
    def build(self, primitives, pForceBalancedTree=True, pAABB=None, pKeepPointInNodes=False, indices=None, normalizeInput=True) :
        ...
        if pAABB is None :
            if normalizeInput :
                self.points = nss_common.applyNormalization(points, nss_common.getAABBox(points), self.pc_translation, self.pc_scaling)
            else :
                self.points = points
    ...
    def farthest_point_samples(self, sample_size) :
        return nss_common.farthest_point_samples(self.points, sample_size)

    def uniform_point_samples(self, sample_size) :
        return nss_common.uniform_point_samples(self.points, sample_size)
    ...
    def importTree(self, file, point_cloud_src_file=None) :
        with open(file, 'rb') as inF :
            ...
            self.max_splits, self.max_nodes = nss_common.num_splits_nodes(self.levels - 1)
            self.__get_build_funcs(self.strategy)

            if point_cloud_src_file is not None :
                with np.load(point_cloud_src_file, allow_pickle=True) as f :
                    pc = f['a']
                    self.points = nss_common.applyNormalization(pc, nss_common.getAABBox(pc), self.pc_translation)
    ...
    def __get_binned_offsets(self, num_bins, offsets, useLinSpace=False, bbox=None) :
        ...
        if useLinSpace :
            ...
            return nss_common.farthest_point_samples(offsets, num_bins)
    ...
    def density(self, samples) :
        ...
        for i, x in enumerate(samples) :
            for leaf in self.leaves_arr :
                if nss_common.isect_point_AABB(x, leaf.aabb) :
    ...
    def abs_diff_pre_order(self, point_cloud, pred_planes, pred_cost, true_cost,
        ...
        root = dummy_node(nss_common.getAABBox(point_cloud), 0, point_cloud)
        ...
        while len(treeStuck) != 0 :
            ...
            if node.lvl == 0:
                ...
                left_aabb = nss_common.refit_aabb(left_aabb)
                right_aabb = nss_common.refit_aabb(right_aabb)

                eval_cost += self.inter_prob_fn(node, root) * self.inter_cost_fn(node)

                idxR = node.index + 1 + nss_common.sumPowerSeries(2, node.lvl - 2)
                idxL = node.index + 1
